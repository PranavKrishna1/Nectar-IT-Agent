...
def _tokenize(text: str) -> set[str]:
    """Lowercase and tokenize text into a set of alphanumeric word tokens."""
...
def rerank(query_text: str, chunks: list[RetrievedChunk], top_n: int = 4) -> list[RetrievedChunk]:
    ...
    ``chunks`` are the results of ``retriever.retrieve``, already above
    the minimum vector-similarity threshold. Returns up to ``top_n``
    chunks, reordered by combined score, with duplicate/near-duplicate
    chunks from the same document collapsed.
    """
    if not chunks:
        return []
