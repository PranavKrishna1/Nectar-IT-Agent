...
def retrieve(query_text: str, top_k: int | None = None) -> list[RetrievedChunk]:
    ...
    Returns:
        Chunks that meet ``settings.rag_min_similarity``, ordered by
        descending similarity. Empty if nothing relevant enough was
        found - callers must treat this as "not found in the knowledge
        base," not silently proceed.
    """
